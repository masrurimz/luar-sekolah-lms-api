...
import type { NewTodo, Todo } from "../-domain/entities";
import type {
	CreateTodoInput,
	GetTodosInput,
	UpdateTodoInput,
...
export abstract class TodoRepository {
	...
	static async update(
		...
		if (Object.keys(updateData).length === 0) {
			return TodoRepository.getById(db, id);
		}
	...
	static async toggle(db: DatabaseType, id: string): Promise<Todo | null> {
		...
		const existingTodo = await TodoRepository.getById(db, id);
		if (!existingTodo) {
			return null;
		}
	 ...
	 */
	static async getByCompletedStatus(
		db: DatabaseType,
		completed: boolean,
		limit = 50,
		offset = 0,
